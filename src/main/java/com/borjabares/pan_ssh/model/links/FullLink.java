...
public class FullLink {
	...
	public FullLink(Links link, long numberOfVotes, boolean voted, User user){
		...
		this.votable = !(link.getStatus() == LinkStatus.DISCARD ||
				link.getStatus() == LinkStatus.BANNED);
		if (user != null){
	...
	public FullLink(Links link, long numberOfVotes, boolean votable, boolean voted,
			boolean mine, boolean editable, boolean userDisposable, boolean adminDisposable) {
		this.link = link;
		this.numberOfVotes = numberOfVotes;
